function openConfirmationDialog(username) {
	...
	confirmDialog.innerHTML = ` < div class = "modal-content" > <p > Are you sure you want to delete user: $ {
		username
	} ? </p>
            <button class="delete-button" onclick="deleteUser('${username}')">Yes</button > <button class = "close-button"onclick = "closeConfirmationDialog()" > No < /button>
        </div > `;
...
function hideUserInfo() {
	...
	element1.style.display = "none"element2.style.display = ""
}

function hidePassChange() {
	var element1 = document.getElementsByClassName('div1')[0];
	var element2 = document.getElementsByClassName('div2')[0];
	element1.style.display = ""element2.style.display = "none"
}